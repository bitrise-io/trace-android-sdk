...
class InputHelper {
  ...
  Set<File> getModuleDirsToUpdate(final Project project, final Set<String> moduleDirNames) {
    ...
    if (moduleDirNames == null || moduleDirNames.isEmpty()) {
      logger.lifecycle(
          "Module name was not specified with property \"{}\", task will generate change "
              + "log entries for all projects",
  ...
  /**
   * Validate that the given input module names are in the available Set of names. If any of
   * the input items are not in the available list, this throws IllegalStateException.
   *
  ...
  void validateModules(final Set<File> availableModules, final Set<String> moduleDirNames) {
    ...
    for (final String moduleDir : moduleDirNames) {
      if (!availableModuleNames.contains(moduleDir)) {
        logger.error(
            "No module found with name \"{}\", task execution is stopping. Please make sure input"
                + " property \"{}\" is set the the name of the given project's directory, or leave "
                + "it blank if you want to generate change log entries for all modules", moduleDir,
            UpdateChangeLogTask.PROPERTY_NAME_MODULES_TO_UPDATE);
        throw new IllegalStateException(
            String.format("Exception when running task, input property \"%s\" is incorrect",
                UpdateChangeLogTask.PROPERTY_NAME_MODULES_TO_UPDATE));
      }
    }
  }
  ...
  Set<File> getAvailableModules(final Project project) {
    return Stream
        .concat(getProjectModules(project).stream(), getProjectIncludedBuilds(project).stream())
        .collect(Collectors.toSet());
  }
