 ...
 * it will update the CHANGELOG.md with the changes made since the last tag. Commits should
 * follow the conventional commits, because this task creates the change log entries based on that.
 * Only commit types "fix", "feat", "fix!" and "feat!" will be inserted to the CHANGELOG.md
 * (these are referred in the code as the allowed commits). When the task is executed, the
 * changes will be uncommitted, you have to commit and push it to the remote repository. Before
 * doing this it is recommended to manually check the changes for any unexpected result, for
...
public class UpdateChangeLogTask extends DefaultTask {
  ...
  /**
   * Does the update of the CHANGELOG.md. All commits since the previous tag will be collected,
   * and the ones with the allowed type will be added to the CHANGELOG.md.
   *
  ...
  private Set<File> processInputs() {
    final Set<File> moduleDirsToUpdate =
        inputHelper.getModuleDirsToUpdate(getProject(), moduleDirNames);
  ...
  private void updateChangeLogWithModule(final File moduleDir) throws IOException, GitAPIException {
    ...
    final List<RevCommit> filteredCommits =
        gitHelper.filterRelevantCommits(git, newCommits, moduleDir);
    logger.lifecycle("Found {} commits affecting \"{}\"", filteredCommits.size(),
        moduleDir.getName());

    final ChangeLogHelper changeLogHelper = new ChangeLogHelper(logger);
    final List<ChangeLogEntry> changeLogEntries =
        changeLogHelper.getChangeLogEntries(filteredCommits);
