...
public class MetricUtilsTest {
  ...
  /**
   * Tests {@link MetricUtils#batchMetrics(Metric...)}, that the batched Metric should be a
   * merge of these Metrics.
   */
  @Test
  ...
  /**
   * Tests {@link MetricUtils#batchMetricsWithName(String, List)}, that Metrics with different
   * name than the given value, should not be merged to the batched value.
   */
  @Test
  public void batchMetricsWithName_shouldNotContainDifferentName() {
    ...
    final Metric actualValue =
        MetricUtils.batchMetricsWithName(metric1.getMetricDescriptor().getName(),
  ...
  /**
   * Tests {@link MetricUtils#isEmpty(Metric)} that should return {@code true} when a Metric with
   * empty name is used.
   */
  @Test
  ...
  /**
   * Tests {@link MetricUtils#groupMetrics(Metric...)} should create a map with the key of the
   * Metric names and the value of the belonging Metrics. The result should contain two groups,
   * 'Sample metric' and 'Different metric', the first with values
   * {@link MetricTestProvider#getSampleMetric()} and
  ...
  public void groupMetrics_shouldBeGroupedByName() {
    ...
    final Metric metric2 = MetricTestProvider.getSampleMetricWithOtherTimeSeries();
    final Metric metric3 = MetricTestProvider.getOtherMetric();
    final Map<String, ArrayList<Metric>> actualValue =
        MetricUtils.groupMetrics(metric1, metric2, metric3);
