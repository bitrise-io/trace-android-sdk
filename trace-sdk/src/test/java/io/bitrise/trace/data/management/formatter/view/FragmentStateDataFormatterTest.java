...
public class FragmentStateDataFormatterTest {
  ...
  /**
   * When the input data timestamp is after all of the timestamps of
   * {@link FragmentDataStateEntry}s, it should return the
   * {@link FragmentStateDataFormatter#INVALID_INDEX}.
   */
  @Test
  ...
  /**
   * The input List of {@link FragmentDataStateEntry}s should be sorted based on their timestamp
   * in ascending order.
   */
  @Test
  public void sortFragmentDataStates_ShouldSortList() {
    ...
    assertThat(fragmentDataStateEntries,
        contains(expectedValue.toArray(new FragmentDataStateEntry[] {})));
  }

  /**
   * When the first {@link FragmentDataStateEntry}s with {@link FragmentState#PAUSED} happens
   * after the first FragmentDataStateEntry with {@link FragmentState#VIEW_CREATED}, it does not
   * have to filter it and should return the same List but ordered.
   */
  @Test
  public void validatePausedEntries_ShouldNotFilter() {
    ...
    assertThat(actualValue,
        contains(pausedFragmentDataStateEntries.toArray(new FragmentDataStateEntry[] {})));
  }

  /**
   * When the some of the {@link FragmentDataStateEntry}s with {@link FragmentState#PAUSED}
   * happens before the first FragmentDataStateEntry with {@link FragmentState#VIEW_CREATED}, it
   * should filter it and should return the filtered List.
   */
  @Test
  ...
  /**
   * When the all of the {@link FragmentDataStateEntry}s with {@link FragmentState#PAUSED}
   * happens before the first FragmentDataStateEntry with {@link FragmentState#VIEW_CREATED}, it
  ...
  /**
   * When there are no {@link FragmentDataStateEntry}s with {@link FragmentState#VIEW_CREATED} it
  ...
  /**
   * When there are no {@link FragmentDataStateEntry}s with {@link FragmentState#PAUSED} it
   * should return {@code false}.
   */
  @Test
  ...
  public void findAllByState_ShouldReturnOnlyViewCreated() {
    ...
    assertThat(actualValue,
        containsInAnyOrder(expectedValue.toArray(new FragmentDataStateEntry[] {})));
  }

  /**
   * Should return from a List of {@link FragmentDataStateEntry}s only the ones with
   * {@link FragmentState#PAUSED}.
   */
  @Test
  ...
  /**
   * From the given {@link FragmentDataStateEntry}s input it should return the expected
   * {@link Span}s as result.
   */
  @Test
