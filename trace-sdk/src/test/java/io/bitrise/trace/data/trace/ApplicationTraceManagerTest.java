...
public class ApplicationTraceManagerTest {
  ...
  /**
   * As {@link TraceActivityLifecycleTracker} is a singleton, if there is any active activity
   * from previous runs, it will stop them.
   */
  @Before
  public void setUpBefore() {
    ...
    traceActivityLifecycleTracker
        .registerTraceActivityLifecycleSink(applicationForegroundStateDataListener);
  ...
  /**
   * When a {@link Span} is added to the active {@link Trace} we should be able to get it back
   * from the Trace.
   */
  @Test
  public void addSpanToActiveTrace_ShouldContainIt() {
    ...
    final Span sampleSpan =
        TraceTestProvider.getSampleSpan(activeTrace.getTraceId(), "dummySpanName");
    ...
    final Span expectedSpan = sampleSpan.toBuilder()
                                        .setTraceId(ByteStringConverter.toByteString(
                                            mockApplicationTraceManager.getActiveTrace()
                                                                       .getTraceId()))
                                        .build();
  ...
  /**
   * When a {@link Span} is added to the active {@link Trace}, but there is no currently active
   * Trace, it should create a new Trace and the Trace ID of the Span should be updated.
   */
  @Test
  ...
  /**
   * When {@link TraceManager#stopTrace()} is not called explicitly during two
   * {@link TraceManager#startTrace()}, startTrace method should call it.
   */
  @Test
  ...
  /**
   * When no {@link Span}s were created for the currently active {@link Trace}, the root Span ID
   * should be {@code null}.
   */
  @Test
  ...
  /**
   * When no root {@link Span} was created for the currently active {@link Trace}, the root Span
   * ID should be {@code null}.
   */
  @Test
  ...
  /**
   * When a root {@link Span}s was created for the currently active {@link Trace}, the root Span
   * ID should be not {@code null}.
   */
  @Test
  ...
  /**
   * When the lifecycle of an activity changes (background/foreground) the ID of the currently
   * active {@link Trace} should change.
   */
  @Test
  ...
  /**
   * Check that the {@link ApplicationTraceManager#getInstance(Context)} returns always the same
   * object.
   */
  @Test
  ...
  /**
   * Check that the {@link ApplicationTraceManager#reset()} resets the {@link TraceManager} and
   * call {@link ApplicationTraceManager#getInstance(Context)} will return a different object on
   * next call.
   */
  @Test
