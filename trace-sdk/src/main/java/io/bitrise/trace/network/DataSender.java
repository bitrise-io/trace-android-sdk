...
public abstract class DataSender extends JobService {
    ...
    public boolean onStopJob(final JobParameters params) {
        setStopped(true);
        final Future<Boolean> future = Executors.newSingleThreadExecutor().submit(this::isRescheduleNeeded);
        try {
            return future.get();
    ...
    /**
     * Validates the given {@link NetworkRequest} if it contains all the required {@link Resource} labels or not.
     *
    ...
    static boolean validateTraceRequest(@Nullable final NetworkRequest networkRequest) {
        ...
        final Set<String> resourceLabels = new HashSet<>(networkRequest.getResource().getLabels().keySet());
     ...
     *
     * @param sessionId the ID of the Session to add as a label, can be {@code null} when used for {@link Metric}s.
     * @return the Resource.
     */
    @NonNull
    @WorkerThread
    Resource getSessionResources(@NonNull final String sessionId) {
        ...
        final List<ResourceEntity> storedResources = getDataStorage().getResourcesWithSessionId(sessionId);
    ...
    /**
     * Removes the {@link Resource}s from the {@link DataStorage}, if they do not have any reference.
     * <p>
     * Should not be called on the main thread.
     *
     ...
     * @param shouldReschedule {@code true} if the sending should be rescheduled, {@code false} otherwise.
     */
    void onSendingFinished(@Nullable final JobParameters jobParameters, final boolean shouldReschedule) {
