...
/**
 * Scheduler implementation. Uses a subclass of the {@link android.app.job.JobScheduler} as the action. The jobs will
 * run by the a {@link android.app.job.JobScheduler}. Service will be executed even when the app is terminated. For
 * details of the JobScheduler please check official docs.
 *
 * @see <a href="https://developer.android.com/reference/android/app/job/JobScheduler">JobScheduler</a>
 */
public class ServiceScheduler extends Scheduler {
    ...
    /**
     * Constructor for class. {@link #DEFAULT_SCHEDULE_INITIAL_DELAY_MS} will be used as delay for the scheduling.
     *
    ...
    public Integer scheduleDelayed(final long initialDelay) {
        final JobInfo jobInfo = new JobInfo.Builder(getJobIdForService(jobService),
                new ComponentName(context, jobService)).setBackoffCriteria(18000, JobInfo.BACKOFF_POLICY_LINEAR)
     ...
     * @return the ID for the given JobService, IllegalArgumentException if it is not a {@link MetricSender} or
     * {@link TraceSender}.
     */
    private int getJobIdForService(@NonNull final Class<? extends JobService> jobService) {
        ...
        throw new IllegalArgumentException("Could not determine the ID for class " + jobService.getName());
    }
}
