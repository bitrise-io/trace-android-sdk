 ...
 * work on Linux based system see linked article. CPU usage data is parsed from "stat" files, see
 * linked manual for the format of these files.
 *
 * @see
...
public class SystemCpuUsageDataCollector extends CpuUsageDataCollector {
  ...
  @NonNull
  @VisibleForTesting
  static CpuUsageData.CpuStat getStatPercentages(@NonNull final CpuUsageData.CpuStat cpuStat,
                                                 final float total) {
  ...
  /**
   * Parses the given String from the system stat File and returns a {@link CpuUsageData.CpuStat}
   * from it.
   *
  ...
  /**
   * Gets the CPU usage for the system and the application. From
   * {@link android.os.Build.VERSION_CODES#O} requires root permission.
   *
   ...
   *
   * <p>An example for proc/stat file content:
   ...
   *
   * <p>As seen, the first line is the total, the rest are for each core.
   *
