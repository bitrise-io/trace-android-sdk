...
/**
 * Listens to the state changes of the Activities of the Application. Gathers and sends the data
 * to the {@link DataManager}.
 *
 * <p>The value of {@link Activity#hashCode()} is used for ID for each Activity, to differentiate
 * between them.
 *
 * @see
 ...
 */
public class ActivityStateDataListener extends BaseDataListener
    implements TraceActivityLifecycleSink {

  /**
   * A Map that identifies a single Activity and attaches the required data to it as an
   * {@link ActivityData}.
   */
  @NonNull
  ...
  public void onActivityCreated(@NonNull final Activity activity,
    ...
    updateActivityMap(activity.hashCode(), activity.getClass().getSimpleName(),
   ...
   * @param name              the name of the Activity, or {@code null} if it does not required
   *                          to change.
  ...
  /**
   * When an Activity reaches a state, that a {@link Span} is finished, we should flush the
   * {@link ActivityData} of the given Activity from the {@link #activityMap} and forward it for
   * further processing.
   *
