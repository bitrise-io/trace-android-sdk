...
public class MetricUtils {
  ...
  /**
   * Batches multiple {@link Metric}s to a single one. Metrics should be batched, when they
   * have the same name.
   *
  ...
  /**
   * Batches multiple {@link Metric}s to a single one. Metrics should be batched, when they
   * have the same name. Metrics with different names will be ignored.
   *
   * @param metrics the Metrics to batch.
   ...
   */
  @NonNull
  public static Metric batchMetricsWithName(@NonNull final String name,
  ...
  /**
   * Flattens the attributes of the merged {@link Metric}s. If a {@link LabelKey} is present
   * multiple times, it removes the duplicates.
   *
  ...
  public static Metric flattenAttributes(@NonNull final Metric.Builder metricBuilder) {
    final Set<LabelKey> labelKeySet =
        new HashSet<>(metricBuilder.getMetricDescriptor().getLabelKeysList());
    final MetricDescriptor.Builder metricDescriptorBuilder =
        metricBuilder.getMetricDescriptor().toBuilder();
  ...
  /**
   * Checks if the given {@link Metric} is empty or not. The Metric is considered empty, when
   * the name of it is empty.
   *
