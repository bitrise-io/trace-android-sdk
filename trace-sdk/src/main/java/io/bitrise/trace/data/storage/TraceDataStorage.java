 ...
 * <br><br>
 * IMPORTANT: To inspect the Room database file use the Database Inspector tool in Android Studio, it is available
 * from version 4.1. Below this version you must extract all 3 files from the device, go to
 * <pre>/data/data/io.bitrise.trace/databases/trace-database*</pre> then rename them to have the correct file
 * extensions like this: "trace-database.db", "trace-database.db-shm" and "trace-database.db-wal". Then open the *.db
 * file in any SQLite browser.
 * <br><br>
 */
@Singleton
public class TraceDataStorage extends DataStorage {
    ...
    private TraceDataStorage(@NonNull final Context context) {
        this.traceDatabase = Room.databaseBuilder(context, TraceDatabase.class, "trace-database").build();
    }
