...
/**
 * Class used by the {@link TraceSdk} that uses Android's ActivityLifecycleCallbacks to monitor Activity lifecycle
 * changes. Can register {@link TraceActivityLifecycleSink}s to forward these events.
 */
@Singleton
public class TraceActivityLifecycleTracker implements Application.ActivityLifecycleCallbacks {
    ...
    @NonNull
    private final static LinkedHashSet<TraceActivityLifecycleSink> registeredTraceActivityLifecycleSinks =
            new LinkedHashSet<>();
    /**
     * A lock object for preventing concurrency issues for the {@link #traceActivityLifecycleTracker}. E.g: when
    ...
    /**
     * A lock object for preventing concurrency issues for the {@link #registeredTraceActivityLifecycleSinks}. E.g: when
     ...
     */
    @NonNull
    public static synchronized TraceActivityLifecycleTracker getInstance(@NonNull final Context applicationContext) {
    ...
    private void unregister() {
        if (applicationContext instanceof Application) {
            ((Application) applicationContext).unregisterActivityLifecycleCallbacks(traceActivityLifecycleTracker);
        }
    }

    /**
     * Registers a {@link TraceActivityLifecycleSink} for the Activity lifecycle events. The registered sinks will
     * receive the events in the order they were registered.
     *
     * @param traceActivityLifecycleSink the given TraceActivityLifecycleSink.
     */
    public void registerTraceActivityLifecycleSink(@NonNull final TraceActivityLifecycleSink traceActivityLifecycleSink) {
        synchronized (registeredTraceActivityLifecycleSinkLock) {
     ...
     * @param traceActivityLifecycleSink the given TraceActivityLifecycleSink.
     */
    public void unregisterTraceActivityLifecycleSink(@NonNull final TraceActivityLifecycleSink traceActivityLifecycleSink) {
        synchronized (registeredTraceActivityLifecycleSinkLock) {
