...
/**
 * {@link DataCollector} type, that collects the Applications usage of the CPU. For information about how CPUs work on
 * Linux based system see linked article. CPU usage data is parsed from "stat" files, see linked manual for the
...
public class ApplicationCpuUsageDataCollector extends CpuUsageDataCollector {
    ...
    static float getUptime(final float cpuStartTime, final float elapsedSeconds) {
        return elapsedSeconds - (cpuStartTime / getNumberOfClockTicksPerSecond(elapsedSeconds, cpuStartTime));
    }

    /**
     * Gets the USER_HZ constant. By default it is 100 for most Android devices, but it can be altered.
     *
     * @param elapsedSeconds the elapsed seconds since boot.
     * @param startTime      The time the process started after system boot. In kernels before Linux 2.6, this value was
     *                       expressed in jiffies.  Since Linux 2.6, the value is expressed in clock ticks (divide by
     *                       sysconf(_SC_CLK_TCK)). This means elapsedTime= ticks/USER_HZ -> USER_HZ =
     ...
     * </ul>
     * <p>
     * If the proc/self/stat file cannot be read, the IOException is caught and logged, and {@code null} is returned.
     *
     * @return the CPU usage percentage, or {@code null} when it cannot be read.
     */
    @Nullable
    @VisibleForTesting
    Double getApplicationCpuUsagePercentage(@NonNull final CpuUsageData.PidCpuStatWithUptime previousPidCpuStatWithUptime) {
        ...
        final double upTimeDiff = pidCpuStatWithUptime.getUptime() - previousPidCpuStatWithUptime.getUptime();
     ...
     * @see
     * <a href="https://man7.org/linux/man-pages/man5/proc.5.html>/href="https://man7.org/linux/man-pages/man5/proc.5.html</a>
     */
    @Nullable
    @VisibleForTesting
    ...
    /**
     * Parses the given String from a given process stat File and returns a {@link CpuUsageData.PidCpuStatWithUptime}
     * from it.
     *
     * @param line the String line to parse.
     * @return the PidCpuStatWithUptime parsed from it, or {@code null} if it could not be read (e.g. CPU offline).
     */
    @Nullable
    @VisibleForTesting
    CpuUsageData.PidCpuStatWithUptime parsePidCpuStat(@NonNull final String line) {
        ...
        try {
            ...
            pidCpuStatWithUptime = new CpuUsageData.PidCpuStatWithUptime(pidCpuStat, uptime, elapsedSeconds);
