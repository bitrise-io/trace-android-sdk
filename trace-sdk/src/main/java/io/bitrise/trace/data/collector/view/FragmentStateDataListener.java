...
/**
 * Class used by the {@link TraceSdk} that uses Android's FragmentLifecycleCallbacks to monitor Fragment lifecycle
 * changes. This class is also a {@link TraceActivityLifecycleSink}, as it needs to be aware of Activity lifecycle
 * changes, to register the FragmentLifecycleCallbacks on them.
 * This also needs the running {@link ActivityStateDataListener} to extract the required {@link ActivityData} from
 * it.
 */
public class FragmentStateDataListener extends FragmentManager.FragmentLifecycleCallbacks implements
    ...
    /**
     * The map that should store every Fragment lifecycle event linked to Activities. Should flush the corresponding
     * data, when an Activity is shut down.
     */
    @VisibleForTesting
    @NonNull
    ...
    /**
     * Lifecycle callbacks instance for code, that use {@link android.app.FragmentManager.FragmentLifecycleCallbacks},
     * which is deprecated, and {@link androidx.fragment.app.FragmentManager.FragmentLifecycleCallbacks} is recommended.
     * Although it is deprecated, we still provide it, so users still using it will be able to use this too.
     */
    @Singleton
    @Nullable
    private volatile android.app.FragmentManager.FragmentLifecycleCallbacks deprecatedFragmentCallbackTracker;
    /**
     * The {@link ActivityStateDataListener} for the class. Required to get the Span IDs of the hosting
     * Activities.
     */
    @NonNull
    ...
    public FragmentStateDataListener(@NonNull final Context context,
                                     @NonNull final ActivityStateDataListener activityStateDataListener) {
    ...
    public void onActivityStopped(@NonNull final Activity activity) {
        ...
        if ((Object) activity instanceof FragmentActivity) {
            ...
            activity.getFragmentManager().unregisterFragmentLifecycleCallbacks(getDeprecatedFragmentCallbackTracker());
        }
     ...
     */
    @VisibleForTesting
    protected void recordFragmentData(@NonNull final Integer fragmentHashCode, @NonNull final FragmentData fragmentData) {
    ...
    /**
     * Gets the {@link android.app.FragmentManager.FragmentLifecycleCallbacks} for receiving and handing Fragment
     * lifecycle events.
     *
    ...
    @NonNull
    synchronized android.app.FragmentManager.FragmentLifecycleCallbacks getDeprecatedFragmentCallbackTracker() {
        if (deprecatedFragmentCallbackTracker == null) {
            deprecatedFragmentCallbackTracker = new android.app.FragmentManager.FragmentLifecycleCallbacks() {
                @Override
                public void onFragmentViewCreated(@NonNull final android.app.FragmentManager fragmentManager,
    ...
    private String getParentSpanIdFromFragmentId(final int fragmentHashCode) {
        final Map<Integer, FragmentData> fragmentRecords = activityFragmentMap.get(activeActivityHashCode);
        if (fragmentRecords != null) {
