 ...
 * <p>
 * Note: this device ID is a GUID, not the real device ID, so with reinstall or with clearing the stored data it could
 * change. However this is still the recommended approach.
 *
 * @see
...
public class DeviceIdDataCollector extends DeviceDataCollector {
    ...
    /**
     * Gets the ID for the device, or creates it if not present. The ID is unique for the given device.
     *
     * @param context the Android Context.
     ...
     *
     * @param context the Android Context.
     * @return the ID for the device, or {@code null}, when the ID cannot be found in the SharedPreferences.
     */
    @Nullable
    private String getDeviceId(@NonNull final Context context) {
        final SharedPreferences sharedPreferences = context.getSharedPreferences(SHARED_PREFERENCES_FILE_NAME,
