...
public class ApplicationTraceManager implements TraceManager {
     ...
     */
    @NonNull
    public synchronized static ApplicationTraceManager getInstance(@NonNull final Context applicationContext) {
    ...
    public void addSpanToActiveTrace(@NonNull final Span span) {
        synchronized (activeTraceLock) {
            ...
            final Span updatedSpan = span.toBuilder()
                    .setTraceId(ByteString.copyFrom(activeTrace.getTraceId(), Charset.defaultCharset()))
                    .build();
