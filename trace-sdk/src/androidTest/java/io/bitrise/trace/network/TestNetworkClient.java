...
public class TestNetworkClient {
   ...
   */
  private static final ConfigurationManager mockConfigurationManager =
   ...
   * The {@link ConfigurationManager} should be mocked for the usage and the token for it should
   * be passed as a param. For testing purposes this is not a singleton.
   *
   ...
   * {@link ConfigurationManager} should be mocked for the usage and the token for it should be
   * passed as a param. The base url for the connection should be 10.0.2.2:9090, as that is the
   * special alias to your host loopback interface. Note: currently it is based on HTTP and not
   * HTTPS. Starting from API level 28 cleartext communication with HTTP will throw an exception.
   *
   * @param token the token to use for the communication.
   * @return the client.
   * @see
  ...
  /**
   * Updates the client with the mock members. This is needed as the {@link ConfigurationManager}
   * is cannot be initialised, so we have to mock it.
   *
   * @param token the token to use for the communication.
   */
