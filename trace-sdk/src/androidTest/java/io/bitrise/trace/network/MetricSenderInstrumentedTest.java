...
public class MetricSenderInstrumentedTest {
  ...
  /**
   * When the {@link MetricSender} is started and it gets a HTTP 200 as result, it should be
   * success.
   *
  ...
  /**
   * When the {@link MetricSender} is started and it gets a HTTP 400 as result, it should be
   * failure.
   *
   * @throws ExecutionException   if the computation threw an exception.
   * @throws InterruptedException if the current thread was interrupted while waiting.
  ...
  /**
   * If the {@link MetricEntity} is sent, and the response is success, the {@link DataStorage}
   * should delete the given MetricEntity.
   *
   * @throws ExecutionException   if the computation threw an exception.
   * @throws InterruptedException if the current thread was interrupted while waiting.
   */
  @Test
  public void send_metricShouldBeDeletedOnSuccess()
    ...
    metricSender.send(null).get();
    final MetricEntity actualResult =
        metricSender.getDataStorage().getMetricById(dummyMetric.getMetricId());

    assertThat(actualResult, is(nullValue()));
  }

  /**
   * If the {@link MetricEntity}s is sent, but the response is failure, the {@link DataStorage}
   ...
   */
  @Test
  public void send_metricShouldNotBeDeletedOnFailure()
      throws ExecutionException, InterruptedException {
    final MetricSender metricSender = mock(MetricSender.class, Mockito.CALLS_REAL_METHODS);
    doNothing().when(metricSender).onSendingFinished(any(), anyBoolean());
    metricSender.setNetworkCommunicator(TestNetworkClient.getFailingCommunicator());
  ...
  /**
   * Tests when the {@link ApplicationUsedMemoryDataCollector}s collected and sent to the
   * TestKit, the result should be {@link HttpURLConnection#HTTP_ACCEPTED}.
   *
  ...
  public void testKit_sendApplicationUsedMemoryMetric() throws IOException {
    ...
    final NetworkCommunicator networkCommunicator =
        TestKitUtils.getNetworkCommunicatorWithValidToken();
  ...
  /**
   * Tests when the {@link SystemMemoryDataCollector}s collected and sent to the TestKit, the
  ...
  /**
   * Tests when the {@link SystemCpuUsageDataCollector}s collected system CPU usage and sent to
   * the TestKit, the result should be {@link HttpURLConnection#HTTP_ACCEPTED}.
   *
   * @throws IOException if any I/O exception occurs.
   */
  @TestKitTest
  ...
  /**
   * Tests when the {@link SystemCpuUsageDataCollector}s collected application CPU usage and sent
   * to the TestKit, the result should be {@link HttpURLConnection#HTTP_ACCEPTED}.
   *
   * @throws IOException if any I/O exception occurs.
   */
  @TestKitTest
