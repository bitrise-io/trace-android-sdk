...
public class TraceSenderInstrumentedTest {
    ...
    /**
     * When the {@link TraceSender} is started and it gets a HTTP 200 as result, it should be success.
     *
    ...
    /**
     * When the {@link TraceSender} is started and it gets a HTTP 400 as result, it should be failure.
     *
     * @throws ExecutionException   if the computation threw an exception.
     * @throws InterruptedException if the current thread was interrupted while waiting.
     */
    ...
    /**
     * If the {@link TraceEntity} is sent, and the response is success, the {@link DataStorage} should delete the
     * given TraceEntity.
     *
     * @throws ExecutionException   if the computation threw an exception.
     * @throws InterruptedException if the current thread was interrupted while waiting.
    ...
    /**
     * If the {@link TraceEntity}s is sent, but the response is failure, the {@link DataStorage} should still hold it.
     *
     * @throws ExecutionException   if the computation threw an exception.
     * @throws InterruptedException if the current thread was interrupted while waiting.
     */
    @Test
    public void send_traceShouldNotBeDeletedOnFailure() throws ExecutionException, InterruptedException {
        final TraceSender traceSender = mock(TraceSender.class, Mockito.CALLS_REAL_METHODS);
        doNothing().when(traceSender).onSendingFinished(any(), anyBoolean());
        traceSender.setNetworkCommunicator(TestNetworkClient.getFailingCommunicator());
        traceSender.setDataStorage(dataStorage);
    ...
    /**
     * When only empty {@link Trace}s are present, {@link TraceSender#getNetworkRequest()} should filter it out.
     */
    @Test
