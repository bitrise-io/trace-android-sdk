...
public class NetworkClientInstrumentedTest {
  ...
  /**
   * Tests the authentication with a valid token. Result code should not be 401. Currently it
   * is 422, as the server cannot parse the entity.
   *
   * <p>This test requires a test kit docker image to run. See root README.md for more details.
   *
  ...
  public void testKit_sendMetricWithValidToken() throws IOException {
    ...
    final MetricRequest metricRequest =
        DataTestUtils
            .getMetricRequest(Collections.singletonList(MetricTestProvider.getEmptyMetric()));
  ...
  /**
   * Tests the authentication with an unexpected token. Result code should be 500, this is a
   * predefined response for this token for testing purposes.
   *
   * <p>This test requires a test kit docker image to run. See root README.md for more details.
   *
   * @throws IOException if any I/O exception occurs.
  ...
  public void testKit_sendMetricWithUnexpectedToken() throws IOException {
    ...
    final NetworkCommunicator networkCommunicator =
        TestKitUtils.getNetworkCommunicatorWithUnexpectedToken();
    final MetricRequest metricRequest =
        DataTestUtils
            .getMetricRequest(Collections.singletonList(MetricTestProvider.getEmptyMetric()));
    final Response<Void> response =
  ...
  public void testKit_sendMetricWithInvalidToken() throws IOException {
    ...
    final NetworkCommunicator networkCommunicator =
        TestKitUtils.getNetworkCommunicatorWithInvalidToken();
    final MetricRequest metricRequest =
        DataTestUtils
            .getMetricRequest(Collections.singletonList(MetricTestProvider.getEmptyMetric()));
    final Response<Void> response =
