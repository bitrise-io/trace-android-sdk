...
public class ApplicationStartUpDataListenerInstrumentedTest {
  ...
  private final Activity mockActivity1 = mock(Activity.class);
  private final Activity mockActivity2 = mock(Activity.class);
  ...
  public void setUp() {
    ...
    traceActivityLifecycleTracker
        .registerTraceActivityLifecycleSink(applicationStartUpDataListener);
    traceActivityLifecycleTracker
  ...
  /**
   * When the first Activity is launched, then put in the background, then re-opened and the
   * onCreate is called, it should be a warm start.
   */
  @Test
  public void handleReceivedData_ShouldBeWarmStart() {
    ...
    final ApplicationStartType actualValue =
        ((ApplicationStartData) dataCaptor.getAllValues().get(1).getContent())
            .getApplicationStartType();
    assertThat(actualValue, is(ApplicationStartType.WARM));
  }

  /**
   * When an Activity is already running and a new Activity is launched, it should not be a
   * startup event.
   */
  @Test
  public void handleReceivedData_ShouldNotBeCalledWhenNewActivityCreated() {
    ...
    traceActivityLifecycleTracker.onActivityStarted(mockActivity1);
    traceActivityLifecycleTracker.onActivityResumed(mockActivity1);
    // Creating the first Activity was an Application launch, so handleReceivedData should be
    // called exactly once.
    ...
    // Launching the second Activity was not an Application launch, so the number of times
    // handleReceivedData was
    ...
    final DataSourceType actualValue =
        dataCaptor.getAllValues().get(0).getDataSourceType();
    assertThat(actualValue, is(DataSourceType.APP_START));
  }

  /**
   * When an Activity is already running and a new Activity is launched, then we navigate back
   * it should not be a startup event.
   */
  @Test
  public void handleReceivedData_ShouldNotBeCalledWhenNavigatingBack() {
    ...
    // After navigating back, the Activity Start lifecycle event should not count as an
    // Application launch, so
  ...
  /**
   * If the data collection is stopped with
   * {@link ApplicationStartUpDataListener#stopCollecting()}, it should not call
   * {@link DataManager#handleReceivedData(Data)}.
   */
  @Test
