...
public class ActivityStateDataListenerInstrumentedTest {
  ...
  /**
   * After an activity is started, {@link ActivityStateDataListener#activityMap} should contain
   * it's ID.
   */
  @Test
  public void activityMap_ShouldContainStartedActivity() {
    traceActivityLifecycleTracker.onActivityStarted(mockActivity);
    assertThat(activityStateDataListener.activityMap.containsKey(mockActivity.hashCode()),
        is(true));
  }

  /**
   * If the data collection is stopped with {@link ActivityStateDataListener#stopCollecting()},
   * it should not put any value to {@link ActivityStateDataListener#activityMap}.
   */
  @Test
  ...
  /**
   * After activity is started, the given Activity's entry in the
   * {@link ActivityStateDataListener#activityMap} should contain the
   * {@link ActivityState#STARTED} value in it's {@link ActivityData}.
   */
  @Test
  public void activityMap_ActivityDataShouldContainStartedState() {
    traceActivityLifecycleTracker.onActivityStarted(mockActivity);
    final ActivityData activityData =
        activityStateDataListener.activityMap.get(mockActivity.hashCode());
    assertThat(activityData.getStateMap().containsKey(ActivityState.STARTED), is(true));
  }

  /**
   * After activity is created, the given Activity's entry in the
   * {@link ActivityStateDataListener#activityMap} should contain the
   * {@link ActivityState#CREATED} value in it's {@link ActivityData}.
   */
  @Test
  ...
  /**
   * After activity is stopped, the given Activity's entry in the
   * {@link ActivityStateDataListener#activityMap} should not contain the
   * {@link ActivityState#STOPPED} value in it's {@link ActivityData}, as it should be flushed.
   */
  @Test
   ...
   * not be called ifmData collection is stopped with
   * {@link ActivityStateDataListener#stopCollecting()}.
   */
  @Test
  ...
  /**
   * If {@link ActivityStateDataListener#stopCollecting()} is called, it should not affect the
   * current state of the {@link ActivityStateDataListener#activityMap}.
   */
  @Test
   ...
   * {@link ActivityStateDataListener#activityMap}. This is required, because during the time the
   * collecting is stopped multiple Activity lifecycle event can occur, which could lead to
   * invalid data.
   */
  @Test
