...
public class SdkInitialisationUiTest {
  ...
  public void startMainActivityFromHomeScreen() {
    ...
    final Intent intent =
        context.getPackageManager()
               .getLaunchIntentForPackage(this.getClass().getPackage().getName());
    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
    context.startActivity(intent);
  }

  /**
   * Checks that on Application launch the initialisation of the Trace SDK should be called at
   * least once. Reads the logcat output and ensures the initialisation log message can be found.
   *
  ...
  /**
   * Checks if the content of the Logcat till the first Activity of the Application is started,
   * that it contains the logging of the initialisation of the SDK.
   *
  ...
  private boolean checkForTraceInitInLogcatContent() throws IOException {
    ...
    String logcatEntry;
    while ((logcatEntry = bufferedReader.readLine()) != null
        && !logcatEntry.contains("TraceSdk: Initialising the SDK)")) {
