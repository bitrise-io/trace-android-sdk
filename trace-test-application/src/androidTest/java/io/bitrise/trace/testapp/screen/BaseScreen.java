...
/**
 * Base Screen class, which should be the parent class for all Screen classes. Contains the
 * common ports for them. Class is used to follow the Page Object pattern.
 *
 * @see
...
public abstract class BaseScreen {
  ...
  /**
   * Performs a click action on the given {@link BySelector}. Attempts to find the given
   * BySelector 3 times. This is needed for cases when the UI is not ready.
   *
  ...
  public void click(@NonNull final BySelector by) {
    for (int i = 0; i < numberOfClickAttempts; i++) {
      ...
      if (uiObject2 != null) {
        ...
        Log.i(BaseUiTest.UI_TEST_TAG,
            String.format("Could not find selector with name %s, retrying %s",
                by.toString(), i));
      }
    }
  }
