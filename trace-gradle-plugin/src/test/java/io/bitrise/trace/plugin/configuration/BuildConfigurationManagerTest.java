...
public class BuildConfigurationManagerTest {
    ...
    public void parseBuildConfiguration_validShouldBeParsed() throws FileNotFoundException {
        ...
        final BuildConfiguration buildConfiguration = BuildConfigurationManager.parseBuildConfiguration(new File(path));
        assertThat(buildConfiguration.getToken(), equalTo("token_abcdef123456"));
        assertThat(buildConfiguration.getVersion(), equalTo("1.0.0"));
    }

    /**
     * Tests {@link BuildConfigurationManager#parseBuildConfiguration(File)} with an invalid JSON file.
     *
     * @throws FileNotFoundException when the JSON file is not found.
     */
    ...
    /**
     * Tests {@link BuildConfigurationManager#parseBuildConfiguration(File)} with a different JSON file.
     *
     * @throws FileNotFoundException when the JSON file is not found.
     */
    @Test(expected = IllegalStateException.class)
    public void parseBuildConfiguration_differentShouldParseNullValues() throws FileNotFoundException {
        final String path = getTestFilePath("not-bitrise-addons-configuration.json");
        BuildConfigurationManager.parseBuildConfiguration(new File(path));
    }

    /**
     * Tests {@link BuildConfigurationManager#parseBuildConfiguration(File)} with wrong path. In this case
    ...
    /**
     * Checks that the {@link BuildConfigurationManager#getInstance(String)} returns a Singleton value.
     */
    @Test
    public void getInstance_assertIsSingleton() {
        final BuildConfigurationManager actualValue = BuildConfigurationManager.getInstance(resourceDir);
    ...
    /**
     * When {@link BuildConfigurationManager#checkBuildConfigVersion(String)} is called with older supported version,
     * then the config file should throw UnsupportedOperationException.
     */
    @Test(expected = UnsupportedOperationException.class)
    public void checkBuildConfigVersion_lowerMajorVersionThrows() {
        final BuildConfigurationManager buildConfigurationManager = BuildConfigurationManager.getInstance(resourceDir);
        buildConfigurationManager.checkBuildConfigVersion("0.1.0");
        buildConfigurationManager.checkBuildConfigVersion("2.0.0");
    }

    /**
     * When {@link BuildConfigurationManager#checkBuildConfigVersion(String)} is called with newer supported version,
     * then the config file should throw UnsupportedOperationException.
     */
    @Test(expected = UnsupportedOperationException.class)
    ...
    /**
     * This test checks rhe integrity of {@link BuildConfiguration}. If this fails, this means, that the config file
     * format has changed, and {@link TraceConstants#TRACE_CONFIGURATION_SUPPORTED_FILE_VERSION} should be updated.
     * After that this file can be updated to make it pass.
     */
    @Test
    public void checkVersionChange() {
        ...
        final String tokenKeyString = "public static final java.lang.String io.bitrise.trace.plugin.configuration" +
                ".BuildConfiguration.TRACE_TOKEN_KEY";
        final String tokenFieldString = "private java.lang.String io.bitrise.trace.plugin.configuration" +
                ".BuildConfiguration.token";
        final String versionFieldString = "private java.lang.String io.bitrise.trace.plugin.configuration" +
