 ...
 * output and some extra indentation. The coloring is based on the ANSI escape codes.
 *
 * @see
...
public class FunctionalTestWriter extends PrintWriter {
  ...
  /**
   * Constructor for class, Requires the given OutputStream and the {@link PrintColor} which
   * will be used.
   *
   * @param outputStream the OutputStream.
   * @param printColor   the given color.
   */
  public FunctionalTestWriter(@NonNull final OutputStream outputStream,
                              @NonNull final PrintColor printColor) {
    super(outputStream);
   ...
   */
  @NonNull
  private static char[] concatCharArrays(@NonNull final char[] first,
  ...
  private static char[] createColorCode(@NonNull final PrintColor printColor) {
    ...
    return String.format("[38;2;%s;%s;%sm", rgb.getRed(), rgb.getGreen(), rgb.getBlue())
                 .toCharArray();
  }
