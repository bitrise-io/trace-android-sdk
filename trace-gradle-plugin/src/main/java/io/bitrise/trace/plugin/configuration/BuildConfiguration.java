 ...
 * {@link io.bitrise.trace.plugin.TraceConstants#BITRISE_ADDONS_CONFIGURATION_FILE} file will be parsed to this Object.
 */
public class BuildConfiguration {

    /**
     * The key for the {@link BuildConfiguration} token value to store in the BuildConfig of the Android project.
     */
    ...
    public BuildConfiguration(@NonNull final String token, @NonNull final String version) {
        if (token == null) {
            throw new IllegalStateException(
                    String.format("Token is missing from %s!", TraceConstants.BITRISE_ADDONS_CONFIGURATION_FILE));
        }
        if (version == null) {
            throw new IllegalStateException(
                    String.format("Version is missing from %s", TraceConstants.BITRISE_ADDONS_CONFIGURATION_FILE));
        }
        this.token = token;
