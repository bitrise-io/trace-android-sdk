...
public class BuildConfigurationManager {
     ...
     */
    @Nullable
    private volatile static BuildConfigurationManager buildConfigurationManager;
    ...
    /**
     * Constructor for the class. Checks if the configuration file exists, can be parsed and the the version of it is
     * supported. Parses the configuration to the {@link #buildConfiguration}.
     *
    ...
    private BuildConfigurationManager(@NonNull final String rootDir) {
        final File jsonFile = new File(rootDir + "/" + TraceConstants.BITRISE_ADDONS_CONFIGURATION_FILE);
        try {
    ...
    static BuildConfiguration parseBuildConfiguration(@NonNull final File file)
        ...
        return new BuildConfiguration(gson.fromJson(new FileReader(file), BuildConfiguration.class));
    }
    ...
    /**
     * Checks if the version of the config file is supported by the plugin or not. Throws UnsupportedOperation if not.
     *
    ...
    void checkBuildConfigVersion(@NonNull final String version) {
        ...
        if (!configFileVersion.isSupported(TraceConstants.TRACE_CONFIGURATION_SUPPORTED_FILE_VERSION)) {
            throw new UnsupportedOperationException(String.format("Current configuration file's version (%1$s) is not" +
                    " supported. Please use same major version of config file or update the Trace plugin " +
                    "version!", version));
        }
    }
