...
public class TraceGradlePlugin implements Plugin<Project> {
    ...
    /**
     * The entry point of the plugin. It's loaded on the first Gradle sync, or when a build starts. Then it's kept in
     * memory (with the Gradle daemon). After evaluation we set up our tasks. Before that point we have no
     * information about the variants and other required information.
     */
    @Override
    public void apply(@NonNull final Project project) {
        buildConfigurationManager = BuildConfigurationManager.getInstance(project.getRootDir().getAbsolutePath());
        beforeEvaluation(project);
        project.afterEvaluate(this::onProjectEvaluated);
    }

    /**
     * Called before project evaluation happens. Sets the Java version to 1.8 and adds InvalidPackage lint exception
     * in the Application that is applied to, to prevent failing because of the GRPC lib dependencies.
     *
    ...
    private void beforeEvaluation(final @NonNull Project project) {
        if (project.getPlugins().hasPlugin(AppPlugin.class)) {
            ...
            if (appExtension == null) {
                throw new IllegalStateException("\"apply plugin: 'com.android.application'\" must be added before " +
     ...
     */
    private void applyPluginOnApplication(@NonNull final Project project, @NonNull final AppExtension appExtension) {
        ...
        appExtension.getTestVariants().forEach(testVariant -> addTraceTasksToVariant(project, testVariant));
        appExtension.getUnitTestVariants().forEach(unitTestVariant -> addTraceTasksToVariant(project, unitTestVariant));
    ...
    /**
     * Adds the Trace Gradle tasks to the given project's given variant. This method should contain the calls that
     * should be made for each variant.
     *
     * @param project the {@link Project}.
     * @param variant the {@link BaseVariant}.
     */
    private void addTraceTasksToVariant(@NonNull final Project project, @NonNull final BaseVariant variant) {
    ...
    /**
     * Adds the Trace Gradle tasks to the given project's given variant's given output. This method should contain
     * the calls that should be made for each variant output.
     *
     * @param project       the {@link Project}.
     * @param variant       the {@link BaseVariant}.
    ...
    /**
     * Registers the {@link ManifestModifierTask} for the builds. Runs after the manifest processor tasks.
     *
     * @param project       the {@link Project}.
     * @param variant       the {@link BaseVariant}.
     * @param variantOutput the {@link BaseVariantOutput}.
    ...
    private void registerManifestModifierTask(@NonNull final Project project,
        ...
        if (variant instanceof TestVariant) {
            project.getLogger().debug(LOGGER_TAG, "Skipping variant {} for manifest modification.", variant.getName());
            return;
        }
        final TaskContainer taskContainer = project.getTasks();
        ...
        final ManifestModifierTask manifestModifierTask =
                (ManifestModifierTask) new TraceVariantTaskBuilder(
                        ...
                        .build();
        final ManifestProcessorTask manifestProcessorTask = TaskUtils.getManifestProcessorTask(variantOutput);
        manifestProcessorTask.finalizedBy(manifestModifierTask);
        final ProcessAndroidResources resourceProcessorTask = TaskUtils.getResourceProcessorTask(variantOutput);
