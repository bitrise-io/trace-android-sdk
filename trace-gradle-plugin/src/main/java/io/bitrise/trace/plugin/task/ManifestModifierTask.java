...
public class ManifestModifierTask extends BaseTraceVariantTask {
    ...
    /**
     * Checks if the given AndroidManifest.xml already have a custom application set, if not the Trace Application
     * will be set.
     *
    ...
    private void addTraceServicesToApplication(@NonNull final ManifestHelper manifestHelper) {
        ...
        if (applicationElement == null) {
            logger.info("No application element found in manifest file {}", manifestHelper.getAndroidManifestPath());
            return;
        }
    ...
    /**
     * Check that all the required permissions for Data collection are present, if not the user will be warned on the
     * build console.
     *
     * @param manifestHelper the {@link ManifestHelper} that holds the given AndroidManifest.xml.
     * @see
    ...
    private void checkPermissions(@NonNull final ManifestHelper manifestHelper) {
        ...
        for (@NonNull final String requiredPermission : requiredPermissions) {
            if (!manifestPermissions.contains(requiredPermission)) {
                logger.warn("{} is missing from {}, Trace plugin won't be able to gather some data. Please check your" +
                                " AndroidManifest.xml(s) and make sure you do not remove these permissions.",
