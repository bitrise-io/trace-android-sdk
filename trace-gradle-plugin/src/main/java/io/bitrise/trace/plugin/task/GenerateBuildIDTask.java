...
/**
 * Generates a build ID text file when the Android APK file is built. The build ID is a timestamp String, displayed
 * in both UTC time and Local time. The text file is placed in the folder of the built APK file.
 */
public class GenerateBuildIDTask extends BaseTraceVariantTask {
    ...
    /**
     * The action that will be performed when the task is run. Will create a build ID an place it in a file.
     */
    @TaskAction
    ...
    /**
     * Creates a build ID.  The Build ID will be the combination of the UTC and local timezone timestamps, with a
    ...
    /**
     * Use the Android Gradle Plugin APIs to find the output folder path of the built Android APK file. This path will
     * be different for Debug and Release builds.
     *
    ...
    private String getBuildOutputFolder() {
        ...
        final String apkFolderPath = baseVariantOutput.getOutputFile().getParentFile().getAbsolutePath();
        logger.debug("APK output folder = " + apkFolderPath);
        return apkFolderPath + "/";
    }

    /**
     * Writes the given build ID to the {@link #BITRISE_BUILD_ID_TXT} build file with the given parent folder.
     *
