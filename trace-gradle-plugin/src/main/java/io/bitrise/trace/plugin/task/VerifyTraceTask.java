...
public class VerifyTraceTask extends BaseTraceTask {
  ...
  /**
   * The action that will be performed when the task is run. Will verify if Trace SDK is
   * correctly set up. Note: we do not need to explicitly check for the configuration file, as it
   * will be checked during evaluation. For more information about this, please see the linked
   * classes/methods.
   *
  ...
  /**
   * Checks if this plugin is correctly set up on the Project or not. Note: we do not need to
   * explicitly report errors, as if the plugin is not applied, the task itself cannot be run.
   *
  ...
  /**
   * Checks if the given {@link Project} has dependency on
   * {@link TraceConstants#TRACE_SDK_DEPENDENCY_NAME} or not. If any
   * {@link org.gradle.api.artifacts.Configuration} that should have it contains it, the
  ...
  private void verifyTraceSdkDependency(@NonNull final Project project) {
    ...
    for (final Configuration configuration : project.getConfigurations()) {
      ...
      if (configurationNameLc.contains("compileclasspath")
          || configurationNameLc.contains("runtimeclasspath")) {
    ...
    if (!hasSdk) {
      throw new IllegalStateException(
          String.format("Project %s does not have dependency on %s, verification failed!",
              project.getName(), TraceConstants.TRACE_SDK_DEPENDENCY_NAME));
    }
  }
