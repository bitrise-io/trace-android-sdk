...
public class TraceVariantTaskBuilder extends TraceTaskBuilder {
    ...
    public BaseTraceVariantTask build() {
        final BaseTraceVariantTask baseTraceVariantTask =
                (BaseTraceVariantTask) taskContainer.create(generatePropertiesForTask(generateTaskName(), type));
    ...
    /**
     * Generates the task name from the {@link #baseName}, {@link #variant} and {@link #variantOutput}. The order
     * will be: <i>VariantOutputName+VariantName+BaseName</i>. It uses {@link FormattingStringBuilder} for the
     * formatting. Note that variant name will be omitted when it is the same with the variant output.
     * <p>
     * Example: When the base name = 'myTask', variant name = 'Variant', variant output name = 'variant', the result
     * will be: <b>'variantMyTask'</b>.
     *
